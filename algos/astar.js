...
async function astar(s, e, graph, m, n, display, animate){
    ...
    f[s[0]*n+s[1]]= calcDistance(s, e) + calcDistance(s, s); // h + g

    var PQ = new PriorityQ();
    
    PQ.enqueue(null, -1*Infinity);
    PQ.enqueue([s,null], -1*f[s[0]*n+s[1]]);
    ...
    while(!PQ.empty()){
        ...
        thisG = f[fi*n+fj] - calcDistance([fi,fj],e);
        ...
        if(fj!=0){
            ni = fi;
            nj = fj-1;
            var newF = calcDistance([ni,nj], e) + thisG+1;  // h + g
            if(!visited[ni*n + nj] && f[ni*n + nj]<newF){
                f[ni*n+nj] = newF;
                PQ.enqueue([[ni,nj], [fi,fj]] , -1*f[ni*n+nj]);

                graph[ni*n + nj]=globalcodes.VISITED; 
            }
        }
        if(fi!=0){
            ni = fi-1;
